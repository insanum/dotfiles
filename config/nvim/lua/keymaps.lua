...
vim.keymap.set('n', 'Zi', '<C-w>_ | <C-w>|',
               { silent = true, noremap = true, desc = 'Zoom window in' })
vim.keymap.set('n', 'Zo', '<C-w>=',
               { silent = true, noremap = true, desc = 'Zoom window out' })

-- toggle spelling
vim.keymap.set('n', ',s', '<cmd>set spell!<CR><cmd>set spell?<CR>',
               { silent = true, desc = 'Toggle spelling' })

-- toggle paste
vim.keymap.set('n', ',v', '<cmd>set paste!<CR><cmd>set paste?<CR>',
               { silent = true, desc = 'Toggle paste' })

-- cut/paste to/from system clipboard
vim.keymap.set('v', ',y', '"+y',
               { silent = true, desc = 'Yank multiple lines (clipboard)' })
vim.keymap.set('n', ',y', '"+y',
               { silent = true, desc = 'Yank motion (clipboard)' })
vim.keymap.set('n', ',Y', '"+yy',
               { silent = true, desc = 'Yank single line (clipboard)' })
vim.keymap.set('v', ',p', '"+p',
               { silent = true, desc = 'Paste overwrite lines (clipboard)' })
vim.keymap.set('n', ',P', '"+P',
               { silent = true, desc = 'Paste before cursor (clipboard)' })
vim.keymap.set('n', ',p', '"+p',
               { silent = true, desc = 'Paste after cursor (clipboard)' })

-- tab stuff
vim.keymap.set('n', '<leader>tn', '<cmd>tabnew<CR>',
               { desc = 'Tab new', noremap = true })
vim.keymap.set('n', '<leader>th', '<cmd>tabprevious<CR>',
               { desc = 'Tab previous', noremap = true })
vim.keymap.set('n', '<leader>tl', '<cmd>tabnext<CR>',
               { desc = 'Tab next', noremap = true })
vim.keymap.set('n', '<leader>tH', '<cmd>tabmove -1<CR>',
               { desc = 'Tab move before', noremap = true })
vim.keymap.set('n', '<leader>tL', '<cmd>tabmove +1<CR>',
               { desc = 'Tab move after', noremap = true })
for i = 1, 9, 1 do
...
vim.keymap.set('n', '<leader>qo', '<cmd>lopen<CR>',
